...
### Node.JS (server-side usage)
  ...
  ```js
  var socket = require('socket.io-client')('http://192.168.1.2');
  socket.on('connect', function(){});
  socket.on('event', function(data){});
  socket.on('disconnect', function(){});
  ```
...
### IO(url:String, opts:Object):Socket
  ...
  `http://192.168.1.2/users`, a transport connection will be established to
  `http://192.168.1.2` and a Socket.IO connection will be established to
  `/users`.
